...
void Register::save() {
    ...
    if(file.open(QIODevice::WriteOnly)) {
        for (auto & itr : Activitylist) {
            memo << itr->getDate().toString("dd.MM.yyyy") << ", " <<itr->getDescription()<<", "<<itr->getStart().toString("HH:mm")<<", "<<itr->getEndTime().toString("HH:mm")<<"\n";
        }
    }
}
...
Activity *Register::find(const QString& a, const QDate& d,const QString& s, const QString& e) {
    for(auto & itr : Activitylist){
        if(itr->getDate()==d && itr->getDescription()==a && itr->getStart().toString("(HH:mm")==s && itr->getEndTime().toString("-HH:mm)")==e)
            return itr;
    }
    return nullptr;
}
